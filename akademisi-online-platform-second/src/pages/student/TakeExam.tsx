...
import { getExamById, submitExam, getStudentResults } from "@/services/exam-service";
  ...
  useEffect(() => {
    const fetchExam = async () => {
      ...
      try {
        ...
        const endTime = new Date(startTime.getTime() + fetchedExam.durationMinutes * 60000);
      ...
      <div className="space-y-6">
        ...
        <Card className="p-6">
          <div className="mb-6 text-sm">
            ...
            <p className="text-red-500 font-medium">Perhatian: Jawablah semua soal sebelum mengirim.</p>
          </div>
              ...
              <div key={question.id} className="border-b pb-6 last:border-b-0">
                ...
                <div className="space-y-2 ml-6">
                  {question.options.map((option, index) => {
                    const answerLetter = String.fromCharCode(65 + index) as "A" | "B" | "C" | "D";
                    return (
                      <div key={index} className="flex items-center">
                        <input
                          ...
                          onChange={() => handleAnswerChange(question.id, answerLetter)}
                          className="mr-2"
                        />
                        <label htmlFor={`${question.id}-${answerLetter}`} className="text-sm">
                          {answerLetter}. {option}
                        </label>
                      </div>
                    );
